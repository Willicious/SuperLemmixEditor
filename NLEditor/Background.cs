...
namespace NLEditor
{
  ...
  class BackgroundList
  {
    ...
    public void SortBackgrounds()
    {
      ...
      {
        if (bg1?.Style == null && bg2?.Style == null) return 0;
        else if (bg1?.Style == null) return -1;
        else if (bg2?.Style == null) return 1;
        else if (!bg1.Style.Equals(bg2.Style)) return bg1.Style.NameInEditor.CompareTo(bg2.Style.NameInEditor);
        else return bg1.Name.CompareTo(bg2.Name);
      });
    }
